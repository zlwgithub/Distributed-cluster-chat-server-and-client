...
public:
    ...
    // 处理登录业务  id  pwd   pwd
    LoginRes Login(string userid, string password)
    {
        LoginRes loginres;
        User user = UserModel::GetInstance().query(userid);
        if (user.getUserId() == userid && user.getPwd() == password)
        {
    ...
    RegRes Register(string username, string password)
    {
        ...
        res.success = UserModel::GetInstance().insert(user);
        if (res.success)
        {
